...
class MainActivity : AppCompatActivity(), Presenter,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /* 网路配置参数View 数据绑定 */
        netParamBinding = DataBindingUtil.inflate<LayoutMainNetConnectionParamBinding>(
            layoutInflater,
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.main_nav_tcp_server ->
                showNetParamDialog(ConnectionParamDao.Type.TCP_SERVER, R.string.main_activity_nav_tcp_server)
            R.id.main_nav_tcp_client ->
                showNetParamDialog(ConnectionParamDao.Type.TCP_CLIENT, R.string.main_activity_nav_tcp_server)
            R.id.main_nav_udp ->
                showNetParamDialog(ConnectionParamDao.Type.UDP, R.string.main_activity_nav_udp)
        }
     ...
     */
    private fun showNetParamDialog(type: ConnectionParamDao.Type, titleRes: Int) {
        model.connectionParamDao.value?.type = type
        netParamBinding.vm = model
        MaterialDialog(this).show {
            title(titleRes)
            customView(view = netParamBinding.root)
